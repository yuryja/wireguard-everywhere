		...
		8)
			...
			until [[ -n "$custom_dns" ]]; do
				...
				for dns_ip in $dns_input; do
					# Check format and validate each octet is 0-255
					if [[ "$dns_ip" =~ ^[0-9]{1,3}(\.[0-9]{1,3}){3}$ ]]; then
						valid_ip=true
						IFS='.' read -ra octets <<< "$dns_ip"
				...
				if [ -z "$custom_dns" ]; then
					echo "Invalid input. Please enter valid IPv4 addresses."
				else
					dns="$custom_dns"
				fi
			done
...
if [[ ! -e /etc/wireguard/wg0.conf ]]; then
	...
	if echo "$ip" | grep -qE '^(10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.|192\.168)'; then
		...
		# Get public IP using HTTPS services with multiple fallbacks
		get_public_ip=$(curl -s -4 https://api.ipify.org 2>/dev/null || wget -qO- -4 https://api.ipify.org 2>/dev/null || curl -s -4 https://icanhazip.com 2>/dev/null || wget -qO- -4 https://icanhazip.com 2>/dev/null)
		# Sanitize the result
		get_public_ip=$(grep -m 1 -oE '^[0-9]{1,3}(\.[0-9]{1,3}){3}$' <<< "$get_public_ip")
